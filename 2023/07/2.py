...
def get_hand_rank(hand, joker='0'):
    counts = Counter(hand)
    if len(counts) == 1:
        ...
        if joker in hand:
            if counts[joker] == 2 or counts[joker] == 3:
                rank = hand_ranks["Four of a Kind"]
            elif counts[joker] == 1 and 3 in counts.values():
                rank = hand_ranks["Four of a Kind"]
            else:
                rank = hand_ranks["Full House"]
...
if __name__ == "__main__":
    ...
    # to have easier sorting we will convert 'JTQKA' to '0ABCD'
    translation = str.maketrans('JTQKA', '0ABCD')
    with open("07/input.txt") as f:
        for line in f.readlines():
            hand, bid = line.split()
            bids[hand.translate(translation)] = int(bid)

    translation = str.maketrans('0ABCD', 'JTQKA')
