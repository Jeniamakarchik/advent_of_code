...
def full_rotation(grid):
    grid = tilt_north(grid) # tilt north
    grid = np.rot90(tilt_north(np.rot90(grid, 1)), 3) # tilt west
    grid = np.rot90(tilt_north(np.rot90(grid, 2)), 2) # tilt south
    grid = np.rot90(tilt_north(np.rot90(grid, 3)), 1) # tilt east
